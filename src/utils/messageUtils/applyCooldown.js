...
module.exports.applyCooldown = (id) => {
  const ApplyCooldownWarning = () => {
    StepLog.info(`Applying cooldown for user(${id})`)
    CooldownWarning.add(id)

    setTimeout(() => {
      StepLog.info(`Removing cooldown for user(${id})`)
      CooldownWarning.delete(id)
    }, 3000)
  }

  if (CooldownWarning.has(id)) {
    StepLog.info(`User(${id}) is trying to use commands too fast after warning about it...`)
    return 3
  }

  if (Cooldown.has(id)) {
    StepLog.info(`User(${id}) is trying to use commands too fast! Putting it into cooldown warning.`)
    ApplyCooldownWarning()

    return 4
  }

  StepLog.info(`Adding cooldown to user(${id})`)
  Cooldown.add(id)
  setTimeout(() => {
    StepLog.info(`Deleting cooldown for user(${id})`)
    Cooldown.delete(id)
  }, 3000)
